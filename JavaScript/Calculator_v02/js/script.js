    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (checkFloat(t) || t === ".") {
            ...
            if (tokens[i-1] === "*") {
                numTokens.pop();
                numTokens.push("^");
            } else if (checkFloat(currentNumber)) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    inputElement.addEventListener("keydown", (event) => {
        try {
            if (event.key === "Enter") {
                ...
                calcElement.textContent = textOutput(tokenize(inputElement.value)) + " = " +result;
    ...
    // Caclulator Buttons
    const calculator = document.querySelector(".calcButtons");
